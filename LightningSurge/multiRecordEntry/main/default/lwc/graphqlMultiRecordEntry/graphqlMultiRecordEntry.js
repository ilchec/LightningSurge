...
import { getCreateContext, getInitialFormValues, navigateToRecord, showToast } from 'c/graphqlMultiRecordEntrySharedUtils';
 ...
 * records of the same object at once. Field presence, grouping, required-ness, and editability
 * come from the object's create-mode page layout (respecting field-level security), same as
 * graphqlRecordForm. All reads/writes use the UI API GraphQL wire adapter - no Apex.
 * @alias GraphqlMultiRecordEntry
 * @extends LightningModal
 * @hideconstructor
 *
 * @example
 * const result = await GraphqlMultiRecordEntry.open({ objectApiName: 'Contact', size: 'large' });
 */
export default class GraphqlMultiRecordEntry extends NavigationMixin(LightningModal) {
  ...
  _queryBridge = new GraphqlQueryBridge();
  _pendingQuery;
  ...
  // See graphqlMultiRecordEntryQueryBridge for how this bridges the reactive query wire to a
  ...
  get isAddRowDisabled() {
    return this._rows.length >= this.maxRows;
  }
  ...
  handleApplyMapping() {
    const mappingDialog = this.template.querySelector('c-graphql-multi-record-entry-mapping');
  ...
  async resolveMatches(batch) {
    if (!this._matchFieldApiNames.length) return new Map();
    const query = buildMatchQuery(this._layoutContext.objectApiName, this._matchFieldApiNames, this._flatColumns, batch);
    if (!query) return new Map();
    const data = await this.runGraphqlQuery(query);
    return extractMatchedIds({ data }, this._layoutContext.objectApiName, this._matchFieldApiNames, batch);
  }
  ...
  handleDone() {
    ...
    const updatedRecordIds = this._saveResults
      ...
      .map((result) => result.detail);
    this.close({ createdRecordIds, updatedRecordIds });
  }
}
